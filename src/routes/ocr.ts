...
3. unit: 품목명·규격 안의 단위(예: 500ml, 1kg)는 무시. 수량 열의 단위 기준으로 판단.
   ...
   - 수량 열 단위가 불명확하면 → "개"
4. unitPrice: 반드시 저장 단위(g, ml, 개) 1단위당 가격(원)으로 환산하여 반환.
   - 공급가액(부가세 제외)이 있으면: 공급가액 ÷ 변환된 amount (소수점 반올림)
   - 공급가액 없이 단가만 있으면:
     - KG/kg → g 변환한 경우: 단가 ÷ 1000
     - L/l → ml 변환한 경우: 단가 ÷ 1000
     - 그 외(개 단위 등): 단가 그대로
   - 단가·공급가액 모두 없으면: null
   - 부가세 금액은 반드시 제외
5. 합계, 부가세, 공급가액 합계, 총액 등 집계 행은 제외. 단, 개별 품목의 공급가액은 unitPrice 계산에 활용.
6. JSON 배열만 반환. 설명 금지.
